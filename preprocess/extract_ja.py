...
def main():
    ...
    vocabfile = sys.argv[2] # '../dataset/mountains_ja.vocab'
    depsfile = sys.argv[3] # '../dataset/mountains_ja.deps'
    ...
    # extract dependency pairs from a knp file.
    # python extract_deps_ja.py -i text.knp.gz -o deps.gz
    # CoNLL: tokens = [(id,form, head,deprel)]
    assert 'gz' in knpfile
    assert 'gz' in depsfile
    with gzip.open(knpfile, mode='rt', encoding='utf8') as ifp,\
